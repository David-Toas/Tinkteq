...
export default function Shipment() {
  ...
  const [filteredShipments, setFilteredShipments] = useState<Shipment[] | null>(null);
  ...
  const handleSearch = () => {
    ...
    const foundShipment = shipments.find((shipment) => shipment.id === searchQuery);
  ...
  return (
    <div className="max-w-screen-sm sm:max-w-md md:max-w-2xl lg:max-w-4xl mx-auto px-4 sm:px-6 py-10">
      <h2 className="text-3xl font-bold text-center">Real-Time Shipment Tracking</h2>
      ...
      <div className="bg-black shadow-lg rounded-lg p-4">
        {(filteredShipments || shipments.slice(0, 3)).map((shipment) => (
          ...
          >
            <div>
              <p className="text-lg font-semibold">Tracking ID: #{shipment.id}</p>
              <p className="text-gray-600"><span className="text-white">Location</span>: {shipment.location}</p>
            </div>
            <p
              className={`text-sm px-3 py-1 rounded-full ${
                shipment.status === "Delivered" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"
              }`}
            >
              🚚 {shipment.status}
            </p>
